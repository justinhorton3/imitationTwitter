	...
	<body>
		...
		<div id="show-tweet" class="content scaffold-show" role="main">
			...
			<ol class="property-list tweet">
				...
				<g:if test="${tweetInstance?.user}">
				<li class="fieldcontain">
					<span id="user-label" class="property-label"><g:message code="tweet.user.label" default="User" /></span>
					
						<span class="property-value" aria-labelledby="user-label"><g:link controller="user" action="show" id="${tweetInstance?.user?.id}">${tweetInstance?.user?.encodeAsHTML()}</g:link></span>
